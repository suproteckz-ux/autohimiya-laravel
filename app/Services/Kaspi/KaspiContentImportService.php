...
class KaspiContentImportService
{
    public function __construct(
        private readonly KaspiEnrichmentParser $parser,
        private readonly KaspiDraftPublisher $publisher,
        private readonly CatalogDescriptionFallbackService $fallbackService,
    ) {
    }
    ...
    public function import(array $options = [], ?AutomationProgressReporterInterface $progress = null): array
    {
        ...
        if (! $dryRun && ! config('services.kaspi.enrichment_enabled')) {
            $this->writeGuardWarningLog($startedAt, $options);

            throw new \RuntimeException('KASPI_ENRICHMENT_ENABLED is not set. Enable it in .env before running mass import.');
        }
        ...
        if (filled($options['product_id'] ?? null)) {
            $query->where('id', (int) $options['product_id']);
        }

        if (filled($options['ids'] ?? null)) {
        ...
        if (filled($options['sku'] ?? null)) {
            $query->where('sku', (string) $options['sku']);
        }
        ...
        $blockedDiagnostics = [];
        $progress->start($total, 'Kaspi: import content.');
        ...
        foreach ($products as $index => $product) {
            ...
            try {
                $effectiveUrl = $kaspiUrl;
                $response = Http::timeout(30)
                    ->withOptions([
                        'on_stats' => function ($stats) use (&$effectiveUrl): void {
                            $uri = method_exists($stats, 'getEffectiveUri') ? $stats->getEffectiveUri() : null;
                            if ($uri) {
                ...
                if (! $response->successful()) {
                    $blockedDiagnostics[] = $this->blockedDiagnostic($product, $response, $effectiveUrl);
                    if (! $dryRun) {
                        $this->updateOrCreateTask($product, 'kaspi_blocked', null, 'HTTP '.$response->status());
                    }
                    $blocked++;
                    $progress->incrementFailed();
                    $progress->advance(1, 'Kaspi: HTTP '.$response->status());
                ...
                if ($dryRun) {
                    $available === 0 ? $noData++ : $imported++;
                    $progress->advance(1, 'Kaspi dry-run: '.$product->id);
                ...
                $progress->advance(1, 'Kaspi: imported product '.$product->id);
                ...
                $errors++;
                $progress->incrementFailed();
                $progress->advance(1, 'Kaspi: product error '.$product->id);
            }
        }

        return [
            'successful' => $errors === 0,
            'warnings' => ($errors + $blocked) > 0,
            'message' => 'Kaspi import complete. Products checked: '.$total,
            'total_items' => $total,
            'processed_items' => $total,
            'updated_count' => $imported + $partial,
            'skipped_count' => $noData,
            'failed_count' => $errors + $blocked,
            'metrics' => compact('imported', 'partial', 'noData', 'blocked', 'errors'),
            'blocked_diagnostics' => $blockedDiagnostics,
        ];
    }
    ...
    private function blockedDiagnostic(Product $product, Response $response, string $effectiveUrl): array
    {
        $requestedUrl = (string) $product->kaspi_product_url;

        return [
            'sku' => $product->sku ?: $product->kaspi_merchant_sku ?: 'product:'.$product->id,
            'http_status' => $response->status(),
            'effective_url' => $effectiveUrl ?: $requestedUrl,
            'content_type' => (string) ($response->header('Content-Type') ?: 'unknown'),
            'response_size' => strlen($response->body()),
            'retry_after' => (string) ($response->header('Retry-After') ?: ''),
            'location' => (string) ($response->header('Location') ?: ''),
            'classification' => $this->blockClassification(
                $response->status(),
    ...
    private function blockClassification(int $status, string $location, string $effectiveUrl, string $requestedUrl): string
    {
        return match (true) {
            in_array($status, [401, 403], true) => 'forbidden',
            $status === 429 => 'rate_limited',
            ($status >= 300 && $status < 400) || filled($location) || ($effectiveUrl !== '' && $requestedUrl !== '' && $effectiveUrl !== $requestedUrl) => 'redirected',
    ...
    private function noDataDiagnostic(Product $product, array $images, int $photosAdded, int $attributesAdded): string
    {
        if ($images !== [] && $photosAdded === 0) {
            return $product->images()->where('source', 'kaspi')->exists() ? 'photos_found_but_duplicate_image' : 'photos_found_but_download_failed_or_duplicate_source';
        }
    ...
    private function writeGuardWarningLog(Carbon $startedAt, array $options): void
    {
        SyncLog::query()->create([
            ...
            'skipped_count' => 0,
            'error_count' => 1,
            'payload_summary' => ['reason' => 'KASPI_ENRICHMENT_ENABLED is disabled.', 'dry_run' => false],
            'diagnostics' => [
